...
**Decision:** rather than force a workaround, this was treated as a legitimate real-world constraint and given its own dedicated, untagged zone instead. See [ADR-002](../04-adr/002-vlan-tagging-vs-separate-networks.md).
...
Metasploitable2 uses the older `ifupdown` system — `/etc/network/interfaces` works directly here, unlike on modern Ubuntu:
