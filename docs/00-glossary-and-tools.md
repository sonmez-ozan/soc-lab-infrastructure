...
Every tool, protocol, and term used across this project, explained in plain language. Organized by category so you can jump to what you need.
...
**rp_filter (Reverse Path Filtering)** — A Linux kernel security feature that can drop packets if the reply would go out a different interface than the request arrived on. Worth checking whenever a host receives a packet but never replies, though in this project the actual cause turned out to be a missing default route, not rp_filter.
...
**Firewall Rule Scope** — Windows Firewall rules can be scoped to "Local subnet only" or "Any IP address." The default "Local subnet" scope silently blocks traffic from a different zone's subnet (like Kali's) even when the rule itself is enabled — this was the actual cause of an RDP/ping failure that looked like a missing rule.
...
**TTL (Time To Live)** — A field in every IP packet that decreases by 1 each time it passes through a router. Seeing a reply's TTL drop by exactly 1 (e.g. 64 → 63) is concrete proof that traffic actually routed through a hop (pfSense) rather than staying on one local segment — used in this project to confirm cross-VLAN routing was genuinely happening, not just that both hosts happened to reach the same gateway.
