...
Real troubleshooting stories from this build — root causes found, not just fixes applied. Kept in the moment they happened rather than reconstructed afterward, since the actual diagnostic process is the valuable part.
...
1. **First attempt:** netplan's `vlans:` section, various key names (`enp0s3.20`, `vlan20`), repeatedly rejected with "unknown key" errors. Multiple netplan file structures tried, including moving the VLAN block between files, fixing a genuine `address` → `addresses` typo along the way, checking `netplan info` for renderer type, and even trying a parallel `systemd-networkd` `.netdev`/`.network` approach (blocked by a `networkctl reload` polkit authorization issue).
...
- **`tun_wg0` never assigned as a real pfSense interface** — this was a genuine missing step (found sitting unassigned under Interfaces → Assignments → Available network ports), fixed by assigning and enabling it — but didn't fix the handshake by itself.
...
- **Clock/timezone skew** — verified pfSense (UTC) and Windows (EDT) were actually the same instant. A dead end here, but worth checking early in general, since clock skew is a very common real cause of WireGuard handshake failures.
...
**Related, still-open observation:** pinging pfSense's own tunnel interface IP through the tunnel continued to time out even after the handshake was confirmed healthy and firewall logs showed the ICMP request PASSING. This is treated as a known self-ping edge case (see [Glossary](00-glossary-and-tools.md)) rather than a real problem, since the meaningful test — a real host-to-host ping through the tunnel — is the one that should be trusted, not a firewall pinging itself.
